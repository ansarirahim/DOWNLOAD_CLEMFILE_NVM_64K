...
int page128;
    ...
    unsigned char pages128[128];
    unsigned char addresslows;
    unsigned char addressmids;
   static int ret = 0;
    static int fd;
    static uint8_t mode;
    ...
    for (ii = 0; ii < 256; ii++)
    {
        for (page128 = 0; page128 < 2; page128++)
        {
            //addresshigh =i;//vPageDetails.at(i).extendedLinearAdderess;// Spi_address >> 16; //
            addressmids = ii; // vPageDetails.at(i).pageNo;//Spi_address >> 8;	 //
            ...
            for (m = 0; m < 128; m++)
            {
                pages128[m] = npages[ii].page[m + addresslows]; ///myNewPages[i].binaryData[m+addresslow];//vPageDetails.at(i).pageData[m+addresslow];
                //npages[tpage].page[x]
